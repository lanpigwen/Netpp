define('helper',['jquery'], function (jQuery) {
  ...
  jQuery.loading = function (showOrClose, options) {
    var msg = /(\/zh|cn\/)/.test(document.URL) ? "鍔犺浇涓�..." : "loading...";
    ...
    function openLoading() {
      ...
      if (!$loadingEl.length) {
        $loadingEl = $('<div id="' + options.loadingID + '"><div class="load-box"><div class="loading">\
        	<div class="spinner"><p>'
          + msg + '</p>	<img src="http://www.huawei.com/Assets/corp/v2/img/huawei_loading.gif"></div>\
...
define('text-dotdotdot',[
  ...
  huawei.text_dot = function () {
    ...
    $(".js-text-dot-cn").filter(":visible").not(".is-truncated").removeAttr("style").dotdotdot({
      wrap: 'letter', //娉細涓枃蹇呴』鏀逛负letter
      ellipsis: "...",
      after: '.toggle',
      callback: dotdotdotCallback
    });
...
$(document).on("videojs-loaded.videojs-language", function(){
  videojs.addLanguage('zh-CN', {
    ...
    "Picture-in-Picture": "鐢讳腑鐢�",
    "Exit Picture-in-Picture": "閫€鍑虹敾涓敾",
    "Mute": "闈欓煶",
    "Unmute": "鍙栨秷闈欓煶",
    ...
    "This modal can be closed by pressing the Escape key or activating the close button.": "鍙互鎸塃SC鎸夐敭鎴栧惎鐢ㄥ叧闂寜閽潵鍏抽棴姝ゅ脊绐椼€�",
    ...
    "Reset": "閲嶇疆",
    "restore all settings to the default values": "鎭㈠鍏ㄩ儴璁惧畾鑷抽璁惧€�",
    "Done": "瀹屾垚",
    "Caption Settings Dialog": "瀛楀箷璁惧畾绐楀彛",
    "Beginning of dialog window. Escape will cancel and close the window.": "鎵撳紑瀵硅瘽绐楀彛銆侲scape閿皢鍙栨秷骞跺叧闂璇濈獥鍙�",
    "End of dialog window.": "缁撴潫瀵硅瘽绐楀彛",
    "Seek to live, currently behind live": "灏濊瘯鐩存挱锛屽綋鍓嶄负寤舵椂鎾斁",
    "Seek to live, currently playing live": "灏濊瘯鐩存挱锛屽綋鍓嶄负瀹炴椂鎾斁",
    "progress bar timing: currentTime={1} duration={2}": "{1}/{2}",
    "{1} is loading.": "姝ｅ湪鍔犺浇 {1}銆�"
  });
});
});
    ...
    (function ($) {
      ...
      $(document).off("click", ".js_video_player, .js-play-btn").on("click", ".js_video_player, .js-play-btn", function (e) {
        if (typeof videojs == "undefined") {
          if (window.console) console.log("player 杩樻湭鍔犺浇");
          return false;
        }
        var playerid = $(this).attr("data-player-id") || "playerContainer";
        var video_path = $(this).attr("data-video-path");
        var video_name = $(this).attr("data-video-name"); // 澧炲姞瑙嗛鍚嶇О
        if (video_name == "" || video_name == undefined) {
          video_name = video_path; //瑙嗛鍚嶇О涓虹┖锛屽幓鍙杙ath
        }
...
define('VideoBox',["plugins/videojs-plugins", "plugins/old-player-compat"], function () {
  ...
  VideoBox.prototype = {
    init: function () {
      ...
      $(div).html('<div class="content"><a href="#" class="btn-close"><span class="iconfont icon-close"></span></a>'
...
define('misc',[
  ...
  // a鏍囩鏂扮獥鍙ｆ墦寮€鐨勯摼鎺ュ鍔爎el="noopener"
...
define('search',[
  ...
  function saveSearchKey(s) {
    ...
    if (validLang(lang)) {
      ...
      var searchkeylistValid = function () {
        return searchkeylist && searchkeylist.length > 0;
      };

      var newsearchkeylist = [];
      newsearchkeylist.push(s);

      if (searchkeylistValid()) {
  ...
  huawei.webchat_share = function () {
    ...
    $.when(wechat_sdk, wechat_auth)
      .done(function (wx, auth) {
        ...
        wx.ready(function () {
          // 鐩戝惉鈥滃彂閫佺粰鏈嬪弸鈥濇寜閽偣鍑汇€佽嚜瀹氫箟鍒嗕韩鍐呭鍙婂垎浜粨鏋滄帴鍙�
          wx.updateAppMessageShareData(options);
          // 鐩戝惉鈥滃垎浜埌鏈嬪弸鍦堚€濇寜閽偣鍑汇€佽嚜瀹氫箟鍒嗕韩鍐呭鍙婂垎浜粨鏋滄帴鍙�
...
define('lazyload',[
	...
	// 鍥剧墖鎳掑姞杞�
	// $("img.lazyload").attr("src", "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk8AEAAFIATgDK/mEAAAAASUVORK5CYII=");
...
define('app/common',['PanelImg', 'vendor/js.cookie-2.2.1.min', 'text-dotdotdot', 'photo',
  ...
  function getHeaderOffsetTop() {
    return ($header.parent().offset() || { top: 0 }).top;
  }
  function fixedHeaderAndS() {
    // 鍥哄畾澶撮儴,浜岀骇瀵艰埅
    if ($(window).scrollTop() >= getHeaderOffsetTop() && !$header.hasClass('fixed')) {
  ...
  function fixedNav() {
    // 鍥哄畾閿氱偣瀵艰埅
    if ($(window).scrollTop() >= ($pointNav.parent().offset() || { top: 0 }).top
     - (($header.hasClass('header-hide') ? 0 : headerHeight) || 0)
  ...
  function fixedTabs() {
    // 鍥哄畾tab鍒囨崲
    if ($(window).scrollTop() >= ($tabAffix.parent().offset() || { top: 0 }).top
     - (($header.hasClass('header-hide') ? 0 : headerHeight) || 0)
  ...
  function resetPointNav() {
    if ($(window).scrollTop() < ($pointNav.parent().offset() || { top: 0 }).top
     - ($header.hasClass('header-hide') ? 0 : headerHeight)
        && $pointNav.hasClass('fixed')) {
  ...
  $tabAffix.wrap('<div class="affix-placeholder" style="height:' + $tabAffix.outerHeight(true) + 'px"></div>');
  ...
  // 瑙ｅ喅IE涓嬭儗鏅棰戣嚜鍔ㄦ挱鏀�,video鏍囩闇€鍔燾lass js-video-autoplay
...
commonModule.GetUserNameInfo = function GetUserNameInfo(Cookies) {
  ...
  var IsBackLogin = Cookies.get("login_uid");// 鍚庡彴鐧诲綍
...
// html鏍囩澧炲姞鐗瑰畾class, attribute锛岄檺瀹氭牱寮忥紝鍏煎娴忚鍣ㄧ敤
